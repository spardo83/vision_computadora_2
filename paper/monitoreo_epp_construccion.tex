...
\author{
\IEEEauthorblockN{Bohórquez, María Gabriela. Ferrari, Maira Daniela. Pardo, Sebastián}
\IEEEauthorblockA{
...
Se desarrolló un sistema de monitoreo automático de EPP que integra detección con YOLO, tracking con ByteTrack y un motor de compliance con lógica de tres estados. A partir del trabajo realizado, se formulan las siguientes conclusiones:

\begin{enumerate}
    \item El modelo YOLOv8m alcanzó el mejor rendimiento global con un mAP50 de 85.8\% y un recall de 78.7\%, superando tanto a YOLOv8n como a YOLOv11n en todas las métricas evaluadas, lo que lo posiciona como la mejor opción cuando se dispone de recursos de cómputo suficientes.
    \item La comparación entre YOLOv8n y YOLOv11n reveló que, a pesar de que YOLOv11n posee menos parámetros (2.6M vs. 3.2M), no logró superar a YOLOv8n en mAP50 (0.769 vs. 0.777), sugiriendo que las optimizaciones arquitectónicas de v11 no compensan la reducción de capacidad en este dominio específico.
    \item El análisis exploratorio del dataset reveló que los elementos de EPP representan menos del 2\% del área de la imagen, lo que constituye el principal desafío para la detección y explica la pérdida del 11--19\% hacia background en las clases NO-X.
    \item La evaluación independiente por EPP con lógica de tres estados (cumple, no cumple, sin dato) demostró ser esencial para evitar falsos positivos de incumplimiento: en el video ``EPP completo'', el compliance pasó de 61\% (evaluación por frame) a 100\% (evaluación por EPP individual), reflejando correctamente la realidad de la escena.
    \item La integración de ByteTrack permitió asignar identidades persistentes a los trabajadores y evaluar su cumplimiento de EPP a lo largo del tiempo, habilitando un sistema de alertas que solo se activa ante incumplimiento sostenido, reduciendo las falsas alarmas causadas por oclusiones momentáneas.
\end{enumerate}
